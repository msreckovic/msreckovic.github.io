...
var DIV_DOCUMENT = "div-document";
var DIV_RULES = "div-rules";
var DIV_POLICIES = "div-policies";
var DIV_PROCEDURES = "div-procedures";
...
function CallbackListPolicies(jsonIn)
{
  var now = Date.now();
  ...
  for (var i=0; i<entries.length; i++) {
    ...
    if (ln) {
      ...
      ln = "http://www.argonautrowingclub.com/" + AutomatedLinksYear + "-policys/?policy=" + sn;
      detail += "<a target=\"_blank\" href=\"" + ln + "\">click for details</a>";
    } else if (active) {
  ...
  var head = "<tr><th>Date</th><th>Policy</th><th>Location</th><th>";
  if (active) {
    head += "Entries Due";
  } else {
    head += "Details";
  ...
  head += "</th></tr>"
  var el = document.getElementById("policys");
  if (el) {
    el.innerHTML = "<table>" + head + total + "</table>";
  }
}

function InfoSinglePolicy(entry)
{
  ...
  total += "</ul>";
  total += "For more details and event list and timing, go to <a target=\"_blank\" href=\"http://policycentral.com\">policycentral.com</a>";
  return total;
}
...
function InfoSinglePayment(entry)
{
  ...
  total += "<li>Payments made after the athlete raced their first race at a policy are subject to a $10 late fee.</li>";
  total += "<li>Payments made after the policy date are subject to a $15 late fee.</li>";
  total += "</ul>";
  return total;
}
...
function InfoSingleFAQ(entry)
{
  ...
  var total = "";
  total += "<ul>";
  total += "<li>You have to be a registered ARC member, as well as registered with RCA. Without that in place, we cannot add you to the crew roster in the policy registration system.</li>";
...
function CallbackForPolicy(entry)
{
  var el;
  ...
  el = document.getElementById("policy");
  if (el) {
    el.innerHTML = InfoSinglePolicy(entry);
  }
...
function CallbackSinglePolicy(jsonIn, index)
{
  var entries = jsonIn.feed.entry;
  CallbackForPolicy(entries[index]);
}

function StandardPolicyConfiguration(shortname, yearsheet, policysheet)
{
  PolicySpecificName = shortname;
  var total = "";
  total += "<h2>Policy Details & Deadlines</h2><div id=\"policy\"></div>\n";
  ...
  var div = document.getElementById("standardpolicy");
  if (div) {
    div.innerHTML = total;
    if (yearsheet) {
      var src = "https://spreadsheets.google.com/feeds/list/" + yearsheet + "/public/values?alt=json-in-script&callback=StandardPolicyCallback";
    ...
    if (policysheet) {
      var src = "https://spreadsheets.google.com/feeds/list/" + policysheet + "/public/values?alt=json-in-script&callback=JsonCallback";
      var script = document.createElement("script");
      script.type = "text/javascript";
      script.src = src;
      div.insertBefore(script, null);
...
function CallbackNamedPolicy(jsonIn, shortname)
{
  var entries = jsonIn.feed.entry;
...
var PolicyName = "";
function SetPolicy(shortname)
{
  PolicyName = shortname;
}

function StandardPolicyCallback(jsonIn)
{
  if (PolicyName == "") {
    ShowAllPolicies(jsonIn);
  } else {
    ShowPolicy(jsonIn, PolicyName);
  }
}
